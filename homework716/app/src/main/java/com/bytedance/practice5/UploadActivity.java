...
public class UploadActivity extends AppCompatActivity {
    ...
    private void submit() {
        ...
        MultipartBody.Part fromconvert = MultipartBody.Part.createFormData("from",USER_NAME);
        MultipartBody.Part toconvert = MultipartBody.Part.createFormData("to",to) ;
        MultipartBody.Part contentconvert = MultipartBody.Part.createFormData("content",content) ;
        MultipartBody.Part imageconvert = MultipartBody.Part.createFormData("image","cover.png",RequestBody.create(MediaType.parse("multipart/form-data"),coverImageData)) ;
        try {
            Call<UploadResponse> ret = api.submitMessage(STUDENT_ID,"",fromconvert,toconvert,contentconvert,imageconvert,token);
            ret.enqueue(new Callback<UploadResponse>() {
                @Override
                public void onResponse(final Call<UploadResponse> call, final Response<UploadResponse> response) {
                    if (!response.isSuccessful()) {
                        return;
                    }
                    final  UploadResponse repo = response.body();
                    if (repo == null ) {
                        return;
                    }
                }
