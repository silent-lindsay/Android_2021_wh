...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator scaleX = ObjectAnimator.ofFloat(target,"scaleX",2f,1f) ;
        scaleX.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        scaleX.setRepeatCount(ValueAnimator.INFINITE);
        scaleX.setInterpolator(new LinearInterpolator());
        scaleX.setRepeatMode(ValueAnimator.REVERSE);

        ObjectAnimator scaleY = ObjectAnimator.ofFloat(target,"scaleY",2f,1f) ;
        ...
        ObjectAnimator toumindu = ObjectAnimator.ofFloat(target,"alpha",1f,0.5f);
        toumindu.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        toumindu.setRepeatCount(ValueAnimator.INFINITE);
        toumindu.setInterpolator(new LinearInterpolator());
        toumindu.setRepeatMode(ValueAnimator.REVERSE);
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleX,scaleY,toumindu);
        animatorSet.start();
    }
}
